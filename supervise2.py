from load_data import load_bank_data, load_breast_cancer_data
...
# Read data
#X, y = load_bank_data('UniversalBank1.csv')
X, y = load_breast_cancer_data('breast_cancer_wisconsin.csv')
...
# plot learning curve
    ...
    plt.legend(loc="best")
    plt.grid()
    plt.savefig('bc_dt_learning_curve.png')
...
# Plot n_samples vs fit_times
    ...
    plt.grid()
    plt.savefig('bc_dt_fit_scability.png')
    ...
    plt.fill_between(fit_times_mean, test_scores_mean - test_scores_std,
                         test_scores_mean + test_scores_std, alpha=0.1)
    plt.grid()
    plt.savefig('bc_dt_fit_performance.png') 
...
if test_DT:
    ...
    plt.legend(loc="best")
    plt.grid()
    plt.savefig('bc_dt_model_complexity_depth.png')
    ...
    plt.legend(loc="best")
    plt.grid()
    plt.savefig('bc_dt_model_complexity_leaf.png')
...
if test_DT:   
    ...
    clf_dt = tree.DecisionTreeClassifier(max_depth=4, min_samples_split=19, random_state=rs)
    train_sizes = np.linspace(0.1, 1.0, 5)
    train_sizes, train_scores, test_scores, fit_times, _ = \
        learning_curve(clf_dt, X_train, y_train, cv=cv, n_jobs=-1, train_sizes=train_sizes, return_times=True)
...
# plot learning curve
    ...
    plt.legend(loc="best")
    plt.grid()
    plt.savefig('bc_NN_learning_curve.png')
...
if test_NN:
    ...
    plt.ylabel("Loss")
    plt.grid()
    plt.savefig('bc_nn_learing_epochs_loss.png')
    ...
    plt.grid()
    plt.legend(loc="best")
    plt.savefig('bc_nn_score_curve.png')
    
  
    # model complexity 
if test_NN:
    #print('NN model complexity starting')
    f1_test = []
    f1_train = []
    ...
    plt.legend(loc='best')
    plt.savefig('bc_nn_model_complexity.png')
...
if test_NN:
    ...
    nnmodel = GridSearchCV(estimator = MLPClassifier(max_iter=10, random_state=rs),
...
if test_NN:   
    ...
    clf_dt = MLPClassifier(solver='sgd', activation='tanh', hidden_layer_sizes=40, learning_rate_init=0.1,random_state=rs)
...
if test_SVM:
    ...
    plt.savefig('bc_svm_model_complexity_rbf.png')
...
if test_SVM:
    ...
    plt.savefig('bc_SVM_Model_complexity_curve.png')
...
if test_KNN:
    ...
    plt.legend(loc='best')
    plt.savefig('bc_KNN_Model_complex_curve.png')
...
if test_KNN:   
    ...
    clf_dt = KNeighborsClassifier(n_neighbors=3, metric='manhattan',weights='uniform')
    train_sizes = np.linspace(0.1, 1.0, 5)
    train_sizes, train_scores, test_scores, fit_times, _ = \
        learning_curve(clf_dt, X_train, y_train, cv=cv, n_jobs=-1, train_sizes=train_sizes, return_times=True)
...
if test_Boosting:
    ...
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig('bc_Boosting_Model_complexity.png')
...
if test_Boosting:   
    ...
    clf_dt = GradientBoostingClassifier(min_samples_leaf=2, max_depth=3, n_estimators=25, learning_rate=0.1, random_state=rs)
    train_sizes = np.linspace(0.1, 1.0, 5)
    train_sizes, train_scores, test_scores, fit_times, _ = \
        learning_curve(clf_dt, X_train, y_train, cv=cv, n_jobs=-1, train_sizes=train_sizes, return_times=True)
...
if times_plot:
    ...
    plt.legend(loc="best")
    plt.savefig('bc_training_accuracy.png')
    ...
    plt.legend(loc="best")
    plt.savefig('bc_testing_accuracy.png')
    ...
    plt.legend(loc="best")
    plt.savefig('bc_training_fit_times.png')
